...
+ Avg(x) has multiplicity mu
  where x has multiplicity x-mu
    ...
    and (x-mu == ZeroOrMore() or x-mu == OneOrMore() or x-mu == OneOrMoreOrdered() or x-mu == ZeroOrMoreOrdered())  else error "Expected multiplicity of higher than One" on x //should be a warning, not an error
